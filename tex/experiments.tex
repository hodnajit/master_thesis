...
Experiments here are more complex, because they are longer both in terms of duration or in terms of travelled distance. These experiments promised to evaluate possible drift caused by the use two relative localization based on IMU and odometry. As I already mentioned in previous chapters, localization based on pure IMU drift with time and odometry drift with travelled distance. Performed experiments can be divided based on 3 simple trajectories of CART2 into
 \begitems
 	* moving in rectangle shape (rectangle test),
