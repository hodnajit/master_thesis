...
where
\begitems
* $f(x_{t-1}, u_t)$ is the nonlinear function that describes the current state of the system based on the previous state and current input,
...
* and $h(x_{t-1}$ is the measurement model based on the previous state $x_{t-1}$.
\enditems
...
where
\begitems
* $g(e_t)$ is resetting function of error state
