...
The localisation is the problem of estimating a robot’s coordinates in an external reference frame from sensor data. In particular, probabilistic approaches are typically more robust in the face of sensor limitations, sensor noise or environment dynamics\cite[probabilistic_robotics]. Moreover, they often scale much better to complex and unstructured environments, where the ability to handle uncertainty is of even greater importance\cite[probabilistic_robotics].

For that reason, only probabilistic methods for state estimation are described in the next chapter. The basis of each technique briefly described in this thesis is Bayes filter\cite[probabilistic_robotics].
...
And third, these algorithms are typically used in the fusion of IMU and GNSS, as I already mentioned in Chapter \ref[chap-introduction].
...
\sec Chosen approach for state estimation
...
The INS meets these requirements and gives us the state independent of a specific vehicle and specific terrain where it is moving. The estimation is accurate for a short time (see Section \ref[IMU]). Thus it needs to be corrected via other measurements as the UWB positioning system and odometry.
...
* The model for estimation of the error is linear, as the error is
	\begitems
		* close to the origin,
