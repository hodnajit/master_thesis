...
}  \right]$ is true skew-symetric matrix (a tensor of angular velocity) [$rad \cdot s^{-2}$],
* $\omega_m$ is angular rate given by gyroscopes [$rad \cdot s^{-2}$],
* $\omega_{bt}$ is true bias of gyroscopes [$rad \cdot s^{-2}$],
* $\omega_n$ is gyroscopes white Gaussian noise [$rad \cdot s^{-2}$],
* and $\omega_w$ is white Gaussian noise gyroscopes bias [$rad \cdot s^{-2}$].
\enditems
...
}  \right]$ is skew-symetric matrix (a tensor of angular velocity) [$rad \cdot s^{-2}$],
* $\omega_m$ is angular rate given by gyroscopes [$rad \cdot s^{-2}$],
* $\omega_{b}$ is bias of gyroscopes [$rad \cdot s^{-2}$].
\enditems
...
* $\delta \omega_b$ is gyroscope bias error [$rad \cdot s^{-2}$],
...
\eqmark
$$
where $h(t)$ is a general nonlinear function of the system state (the true navigation state), and $\rho$ is a white Gaussian noise with covariance.  For UWB localization the function is simple as it is
$$
y = p_t + \rho_1,
\eqmark
$$
with covariance $Q_1$. But for odometry it is a little bit complicated
$$
y = R_t^{-1}v_t + \rho_2
\eqmark
$$
with covariance $Q_1$. This difference is important in computation of Jacobian for ES-EKF algorithm.

\secc The kinematics equations in discrete time
As the equations in continous time are derived from book \cite[quaternion_kinematics], where their representation in discrete time are also presented, I decided to write down only parts, which are different. For more detail see equations 260 in \cite[quaternion_kinematics]. The equation 26c is a bit different, since I am using rotation matrix for orientation representation and not quaternion. This equation is changed to
$$
