...
where
\begitems
* $p_t$ is the true position in 3D [$m$],
* $v_t$ is the true linear velocity in 3D [$m \cdot s^{-2}$],
* $R_t$ is the true rotation matrix of orientation,
* $a_m$ is the specific force given by accelerometers [$m \cdot s^{-2}$],
* $a_{bt}$ is the true accelerometer bias [$m \cdot s^{-2}$],
* $a_n$ is the accelerometers white Gaussian noise [$m \cdot s^{-2}$],
* $a_w$ is the white Gaussian noise accelerometers bias [$m \cdot s^{-2}$],
* $g_t$ is the true gravity vector [$m \cdot s^{-2}$],
...
}  \right]$ is the true skew-symetric matrix (a tensor of angular velocity) [$rad \over s$],
* $\omega_m$ is the angular rate given by gyroscopes [$rad \over s$],
* $\omega_{bt}$ is the true bias of gyroscopes [$rad \over s$],
* $\omega_n$ is the gyroscope's white Gaussian noise [$rad \over s$],
* and $\omega_w$ is the white Gaussian noise gyroscope's bias [$rad \over s$].
\enditems

The state $x_t$, is governed by IMU noisy reading $u_m$ and perturbed by the white Gaussian noise $w$, defined by
...
where
\begitems
* $p$ is the position in 3D [$m$],
* $v$ is the linear velocity in 3D [$m \cdot s^{-2}$],
* $R$ is the rotation matrix of orientation,
* $a_m$ is the specific force given by accelerometers [$m \cdot s^{-2}$],
* $a_{b}$ is the accelerometer bias [$m \cdot s^{-2}$],
* $g$ is the gravity vector [$m \cdot s^{-2}$],
...
}  \right]$ is the skew-symetric matrix (a tensor of angular velocity) [$rad \over s$],
* $\omega_m$ is the angular rate given by gyroscopes [$rad \over s$],
* $\omega_{b}$ is the bias of gyroscopes [$rad \over s$].
\enditems
...
* $\delta a_b$ is the acceleration bias error [$m \cdot s^{-2}$],
* $\delta \omega_b$ is the gyroscope bias error [$rad \over s$],
* $\delta g$ is the gravity vector error [$m \cdot s^{-2}$],
* $R$ is the rotation matrix given by the nominal state,
* $a_m$ is the specific force given by accelerometers [$m \cdot s^{-2}$],
* $a_{b}$ is the accelerometer bias [$m \cdot s^{-2}$],
* $a_n$ is the accelerometers white Gaussian noise [$m \cdot s^{-2}$],
* $a_w$ is the white Gaussian noise accelerometers bias [$m \cdot s^{-2}$],
* $\omega_m$ is the angular rate given by gyroscopes [$rad \over s$],
* $\omega_{b}$ is the bias of gyroscopes [$rad \over s$],
* $\omega_n$ is the gyroscopes white Gaussian noise [$rad \over s$],
* and $\omega_w$ is the white Gaussian noise gyroscopes bias [$rad \over s$].
.
\enditems
...
where
\begitems
* $i$ is a perturbation vector (usually modeled as white Gaussian noise).
\enditems
...
* $F_x$ is a transition matrix,
* $F_i$ is the Jacobian of error state system by impulses,
* $Q$ is the covariances of process noise,
\enditems

The {\bf transition matrix}\fnote{Called system matrix in some literature} $F_x$ is error state Jacobian and it is simply determined by error state kinematics equations $f(\delta x_t)$ in discrete time in Section \ref[chap-secc-kin-discr],
$$
\eqalign{
...
0 & 0 & R\{\omega_m-\omega_b\}^T \Delta t & 0 & -I \Delta t & 0 \cr
0 & 0 & 0 & I & 0 & 0 \cr
0 & 0 & 0 & 0 & I & 0 \cr
0 & 0 & 0 & 0 & 0 & I}
\right].
...
where 
\begitems
* $\sigma_{a_n}$ is the standard deviation of accelerometers [$m \cdot s^{-2}$],
* $\sigma_{\omega_n}$ is the standard deviation of accelerometers [$rad \over s$]
* $\sigma_{a_w}$ is the velocity random walk [$rad \over s \sqrt{s}$],
* $\sigma_{\omega_w}$ is the angular random walk [$rad \over s \sqrt{s}$].
\enditems
...
\delta x &\leftarrow K(y-h(\hat{x})) \cr
...
* $\delta x$ is an error state.
\enditems

The {\bf observation matrices} differ for {\it UWB localization} ($H_1$) and {\it odometry} ($H_2$)
$$
\eqalign{
H_1 &=  \left[
\matrix{
...
where
\begitems
* $R_t$ is the orientation in navigation state,
* $v_t$ is the linear velocity in navigation state,
* $\Theta$ is the orientation $R_t$ in rotation vector form,
* $J_r$ is the right Jacobian of rotation group $SO(3)$ (see equation 183 in \cite[quaternion_kinematics]).
\enditems
...
$$
\eqalign{
\delta x &\leftarrow 0 \cr
P &\leftarrow G P G^T 
}
\eqmark
$$
where $G$ is the Jacobian matrix defined as
$$
G = \left[
\matrix{
I_6 & 0 & 0 \cr
0 & I - [{ 1 \over 2 } \delta \Theta]_x & 0 \cr
0 & 0 & I_9
}
\right].
\eqmark
...
{\bf ROS2\cite[ros2]} is a set of software libraries and tools for building robot applications. It is open-source, and it consists of drivers for hardware, state-of-the-art algorithms, tools for debugging, visualization, simulation, communications overall processes. All applications created in ROS2 are easy to share and are used in the community. It supports the most known and most used programming languages like C++, Python, Java, Lua or Lisp. ROS2 distributions are released to work on operating systems like Ubuntu, MacOs or Windows. Nevertheless, as it is open-source, users usually use it with one Ubuntu distribution, such as 20.04 or 18.04.
...
ROS2 has defined code style and languages version which are recommended to use. The implementation sticks to these rules and uses C++17 and Python3. As the localization needs to be implemented as a real-time application, it is implemented in C++17. Python3 is used for the visualization of experiments results and supporting scripts.
